...
/* bm_utils.c */
extern char	*env_config PROTO((char *var, char *dflt));
extern long	yes_no PROTO((char *prompt));
extern void     a_rnd PROTO((int min, int max, int column, char *dest));
extern int     tx_rnd PROTO((long min, long max, long column, char *tgt));
extern long	julian PROTO((long date));
extern long	unjulian PROTO((long date));
extern FILE	*tbl_open PROTO((int tbl, char *mode));
extern long	dssncasecmp PROTO((char *s1, char *s2, int n));
extern long	dsscasecmp PROTO((char *s1, char *s2));
extern int		pick_str PROTO((distribution * s, int c, char *target));
extern void	agg_str PROTO((distribution *set, long count, long col, char *dest));
extern void	read_dist PROTO((char *path, char *name, distribution * target));
extern void	embed_str PROTO((distribution *d, int min, int max, int stream, char *dest));
#ifndef STDLIB_HAS_GETOPT
extern int		getopt PROTO((int arg_cnt, char **arg_vect, char *oprions));
#endif /* STDLIB_HAS_GETOPT */
extern DSS_HUGE	set_state PROTO((int t, long scale, long procs, long step, DSS_HUGE *e));
extern void gen_phone (DSS_HUGE ind, char *target, long seed);
extern void dss_random64(DSS_HUGE *tgt, DSS_HUGE nLow, DSS_HUGE nHigh, long nStream);
extern DSS_HUGE rpb_routine(DSS_HUGE p);
/*
* Function prototypes
*/
extern void	usage (void);
extern int		prep_direct (char *);
extern int		close_direct (void);
extern void	kill_load (void);
extern int		pload (int tbl);
extern void	gen_template (int tnum, DSS_HUGE start, DSS_HUGE count, long upd_num);
extern void	gen_component_tuplelevel  (int arity, long idx);
extern void	gen_component_attrlevel   (int arity, long idx);
extern void	gen_components (void);
...
#else
#define  PR_DATE(tgt, yr, mn, dy)	\
    sprintf(tgt, "19%02ld-%02ld-%02ld", (long) yr, (long) mn, (long) dy)
#endif /* DATE_FORMAT */
